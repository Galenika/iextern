...
struct ImVec4
{
	...
	float operator[ ](unsigned int i) const { return ((float*)this)[i]; }
...
namespace ImGui
{
	...
	IMGUI_API void          LoadSettings(const char* filename);
	IMGUI_API void          SaveSettings(const char* filename);
	...
	IMGUI_API bool          SliderInt(const char* label, int* v, int v_min, int v_max, const char* display_format = "%.0f", ImVec4 color = ImVec4(0, 0, 0, 0));
	...
	IMGUI_API bool          Selectable1(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));  // size.x==0.0: use remaining width, size.x>0.0: specify width. size.y==0.0: use label height, size.y>0.0: specify height
	...
	IMGUI_API bool			ColorPicker(const char* label, float* col);
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_SenyaPidoras = 1 << 16,
	ImGuiWindowFlags_AlwaysUseWindowPadding = 1 << 17,  // Ensure child windows without border uses style.WindowPadding (ignored by default for non-bordered child windows, because more convenient)
														// [Internal]
...
struct ImGuiIO
{
	...
	const char*   ini_filename;              // = "imgui.ini"        // Path to .ini file. NULL to disable .ini saving.
...
struct ImGuiTextFilter
{
	...
	bool                Draw(const char* label = "Search for ID", float width = 0.0f);    // Helper calling InputText+Build
