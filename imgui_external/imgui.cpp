...
//	WindowTitleAlign = ImGuiAlign_Left;// Minimum window size
	...
	Colors[ImGuiCol_Text] = ImVec4(0.90f, 0.90f, 0.90f, 1.00f); //large gay
	...
	Colors[ImGuiCol_TitleBg] = ImVec4( 0.31f, 0.41f, 0.51f, 0.83f);
	Colors[ImGuiCol_TitleBgCollapsed] = ImVec4( 0.3f, 0.3f, 0.4f, 1.0f );
	Colors[ImGuiCol_TitleBgActive] = ImVec4( 0.3f, 0.3f, 0.4f, 1.0f );
	Colors[ImGuiCol_MenuBarBg] = ImVec4( 0.3f, 0.3f, 0.4f, 1.0f );
	Colors[ImGuiCol_ScrollbarBg] = ImVec4(0.40f, 0.40f, 0.80f, 0.60f);
	Colors[ImGuiCol_ScrollbarGrab] = ImVec4(0.40f, 0.40f, 0.70f, 0.30f);
	Colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(0.40f, 0.40f, 0.80f, 0.40f);
	Colors[ImGuiCol_ScrollbarGrabActive] = ImVec4(0.80f, 0.50f, 0.50f, 0.40f);
	Colors[ImGuiCol_ComboBg] = ImVec4(0.20f, 0.20f, 0.20f, 0.99f);
	Colors[ImGuiCol_CheckMark] = ImVec4(0.90f, 0.90f, 0.90f, 0.50f);
	Colors[ImGuiCol_SliderGrab] = ImVec4( 0.61f, 0.61f, 0.94f, 0.83f );
	Colors[ImGuiCol_SliderGrabActive] = ImVec4( 0.65f, 0.65f, 0.98f, 0.83f );
	Colors[ImGuiCol_Button] = ImVec4( 0.13f, 0.23f, 0.33f, 0.83f ); // 0.67f, 0.40f, 0.40f, 0.60f / 0.04f, 0.65f, 0.65f, 0.84f
	Colors[ImGuiCol_ButtonHovered] = ImVec4( 0.17f, 0.27f, 0.37f, 0.83f ); // 0.67f, 0.40f, 0.40f, 1.00f / 0.04f, 0.59f, 0.59f, 0.84f
	Colors[ImGuiCol_ButtonActive] = ImVec4( 0.21f, 0.31f, 0.41f, 0.83f ); // 0.80f, 0.50f, 0.50f, 1.00f / 0.04f, 0.75f, 0.75f, 0.90f
...
void ImGui::NewFrame()
{
	...
	if (!g.Initialized)
	{
		...
		LoadIniSettingsFromDisk(g.IO.ini_filename);
		g.Initialized = true;
	}
	...
	if (g.SettingsDirtyTimer > 0.0f)
	{
		...
		if (g.SettingsDirtyTimer <= 0.0f)
			SaveIniSettingsToDisk(g.IO.ini_filename);
	}
...
static bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags extra_flags)
{
	...
	if (extra_flags == ImGuiWindowFlags_SenyaPidoras)// eto pravda
	{
		...
		parent_window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y), ImVec2(child_bb.x, child_bb.y + size_arg.y), ImColor(205, 205, 205));
		parent_window->DrawList->AddLine(ImVec2(child_bb.x + size_arg.x, child_bb.y), ImVec2(child_bb.x + size_arg.x, child_bb.y + size_arg.y), ImColor(205, 205, 205));
		parent_window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y + size_arg.y), ImVec2(child_bb.x + size_arg.x, child_bb.y + size_arg.y), ImColor(205, 205, 205));
		parent_window->DrawList->AddLine(ImVec2(child_bb.x, child_bb.y), ImVec2(child_bb.x + 10, child_bb.y), ImColor(205, 205, 205));
		parent_window->DrawList->AddLine(ImVec2(child_bb.x + 20 + value_bufsize.x, child_bb.y), ImVec2(child_bb.x + size_arg.x, child_bb.y), ImColor(205, 205, 205));
		parent_window->DrawList->AddText(ImVec2(child_bb.x + 15, child_bb.y - 6), ImColor(205, 205, 205), name);
	...
	if (first_begin_of_the_frame)
	{
		...
		else
		{
			...
			if (!(flags & ImGuiWindowFlags_NoTitleBar))
				...
				window->DrawList->AddRectFilledMultiColor(title_bar_rect.GetTL(), title_bar_rect.GetBR(), GetColorU32(ImGuiCol_TitleBg), GetColorU32(ImGuiCol_TitleBg), GetColorU32(ImGuiCol_TitleBgActive), GetColorU32(ImGuiCol_TitleBgActive));
			// Menu bar
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			...
			RenderTextClipped(text_min, text_max, name, NULL, &text_size, ImGuiAlign_Center);
		}
...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags )
{
	...
	auto color_red = ImGui::GetColorU32( ImVec4( 0.66, 0.66, 0.66, 0.65 ) );
	auto color_blue = ImGui::GetColorU32( ImVec4( 0.33, 0.33, 0.33, 0.45 ) );
	...
	window->DrawList->AddRect( bb.Min, bb.Max, GetColorU32( ImVec4( 0.88f, 0.88f, 0.88f, 0.88f ) ) );

	window->DrawList->AddRectFilledMultiColor( bb.Min, bb.Max, color_red, color_red, color_blue, color_blue ); // Main gradient. gay
...
bool ImGui::DragFloat(const char* label, float* v, float v_speed, float v_min, float v_max, const char* display_format, float power)
{
	...
	char value_buf[64];
	const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), display_format, *v);
	RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImGuiAlign_Center);
...
void ImGui::ProgressBar(float fraction, const ImVec2& size_arg, const char* overlay)
{
	...
	if (overlay_size.x > 0.0f)
		RenderTextClipped(ImVec2(ImClamp(fill_br.x + style.ItemSpacing.x, bb.Min.x, bb.Max.x - overlay_size.x - style.ItemInnerSpacing.x), bb.Min.y), bb.Max, overlay, NULL, &overlay_size, ImGuiAlign_Center | ImGuiAlign_VCenter);
}
...
bool ImGui::Spinner(const char* label, float radius, int thickness, const ImU32& color) {
	...
	int num_segments = 30;
	int start = abss(ImSin(g.Time*1.8f)*(num_segments - 5));
...
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
	...
	if (*current_item >= 0 && *current_item < items_count)
	{
		...
		if (items_getter(data, *current_item, &item_text))
			RenderTextClipped(frame_bb.Min + ImVec2(2, 0), value_bb.Max, item_text, NULL, NULL);
	}
...
bool ImGui::Selectable1(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	RenderTextClipped(bb.Min, bb_with_spacing.Max, label, NULL, &label_size, ImGuiAlign_Center);
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg)
{
	...
	RenderTextClipped(frame_bb.Min + style.FramePadding, frame_bb.Max - style.FramePadding, buf_display, NULL, &label_size, ImGuiAlign_Center | ImGuiAlign_VCenter);
